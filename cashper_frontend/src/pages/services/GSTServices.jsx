...
const GSTServices = ({ isPopupMode = false, onPopupClose }) => {
  ...
  const [applicationForm, setApplicationForm] = useState({
    ...
    businessName: '', gstin: '', serviceType: '', turnover: '',
    address: '', city: '', state: '', pincode: '',
    gstCertificate: null, salesInvoices: null, purchaseInvoices: null, bankStatements: null, panCard: null
  });
  ...
  useEffect(() => {
    ...
    const savedFormData = sessionStorage.getItem('gst_services_form_data');
    const savedStep = sessionStorage.getItem('gst_services_pending_step');
    ...
    if (savedFormData && token) {
      try {
        ...
        sessionStorage.removeItem('gst_services_form_data');
        sessionStorage.removeItem('gst_services_pending_step');
  ...
  const handleHeroFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/corporate-inquiry/gst-services', {
        method: 'POST',
      ...
      if (response.ok) {
        toast.success(data.message || 'Thank you! Our GST expert will contact you soon.');
  ...
  const validateField = (name, value) => {
    ...
    switch(name) {
      ...
      case 'businessName':
        ...
        break;
      case 'gstin':
        if (value.trim() && !/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/.test(value.toUpperCase())) error = 'Invalid GSTIN format';
        break;
      case 'serviceType':
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      ['businessName', 'serviceType', 'turnover'].forEach(field => {
        const error = validateField(field, applicationForm[field]);
        if (error) stepErrors[field] = error;
      });
    } else if (step === 3) {
  ...
  const nextStep = () => {
    if (currentStep === 1 && !isAuthenticated) {
      sessionStorage.setItem('gst_services_pending_step', '2');
      sessionStorage.setItem('gst_services_form_data', JSON.stringify(applicationForm));
      toast.warning('Please login to continue with your GST services application', {
        position: 'top-center',
        autoClose: 3000
      });
      navigate('/login?redirect=/services/gst-services&step=2', {
        state: { from: '/services/gst-services', returnStep: 2 }
      });
      return;
    }
  ...
  const handleApplicationSubmit = async (e) => {
    ...
    if (!isAuthenticated) {
      sessionStorage.setItem('gst_services_pending_step', '4');
      sessionStorage.setItem('gst_services_form_data', JSON.stringify(applicationForm));
      toast.error('Please login to submit your application');
      navigate('/login?redirect=/services/gst-services&step=4', {
        state: { from: '/services/gst-services', returnStep: 4 }
      });
      return;
    }
    ...
    try {
      // Call API to submit GST services application
      const response = await submitGSTServices(applicationForm);
      
      if (response.success) {
        setShowSuccessModal(true);
        toast.success('GST services application submitted successfully!');
        setApplicationForm({
          fullName: '', email: '', phone: '', panNumber: '',
          businessName: '', gstin: '', serviceType: '', turnover: '',
          address: '', city: '', state: '', pincode: '',
  ...
  const features = [
    { icon: FaFileInvoice, title: "GST Registration", desc: "New & voluntary registration" },
    { icon: FaCalculator, title: "Monthly Returns", desc: "GSTR-1, GSTR-3B filing" },
    { icon: FaMoneyBillWave, title: "GST Refunds", desc: "Export & ITC refunds" },
    { icon: FaPercentage, title: "GST Advisory", desc: "Rate determination & ITC" },
  ];

  const gstRates = [
    { rate: "0%", items: "Essential items: Milk, bread, vegetables" },
    { rate: "5%", items: "Common use: Sugar, tea, edible oil" },
    { rate: "12%", items: "Processed foods, computers" },
    { rate: "18%", items: "Most goods: Soaps, IT services" },
    { rate: "28%", items: "Luxury: Cars, ACs, cigarettes" },
  ];
            ...
            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
              <div className="bg-gradient-to-r from-green-600 to-green-700 px-6 sm:px-8 py-6 sm:py-8">
                <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white text-center">
                  GST Services Application
                </h2>
                <p className="text-green-50 text-center mt-2">Complete the form to get expert GST assistance</p>
              </div>
              ...
              <div className="px-6 sm:px-8 py-8">
                ...
                {currentStep === 2 && (
                  <div className="space-y-6">
                    <h3 className="text-xl font-bold text-gray-900 mb-6">GST Service Details</h3>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        name="gstin"
                        value={applicationForm.gstin}
                        onChange={handleApplicationChange}
                        onBlur={() => handleBlur('gstin')}
                        className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none transition-colors uppercase ${
                          errors.gstin ? 'border-red-500 focus:border-red-600' : 'border-gray-200 focus:border-green-500'
                        }`}
                      ...
                      {errors.gstin && (
                        <p className="mt-1 text-sm text-red-600 flex items-center">
                          <AlertCircle className="w-4 h-4 mr-1" /> {errors.gstin}
                        </p>
                      )}
                    ...
                    <div>
                      <label className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                        <FaCalculator className="mr-2 text-green-600" /> Annual Turnover *
                      </label>
                      <input
                        type="number"
                        name="turnover"
                        value={applicationForm.turnover}
                        onChange={handleApplicationChange}
                        onBlur={() => handleBlur('turnover')}
                        className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none transition-colors ${
                          errors.turnover ? 'border-red-500 focus:border-red-600' : 'border-gray-200 focus:border-green-500'
                        }`}
                        placeholder="Enter annual turnover in ₹"
                      />
                      {errors.turnover && (
                        <p className="mt-1 text-sm text-red-600 flex items-center">
                          <AlertCircle className="w-4 h-4 mr-1" /> {errors.turnover}
                        </p>
                      )}
                    </div>
                  </div>
                ...
                {currentStep === 4 && (
                  <div className="space-y-6">
                    ...
                    <div>
                      <label className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                        <Upload className="mr-2 text-green-600" /> GST Certificate *
                      </label>
                      <input
                        type="file"
                        name="gstCertificate"
                      ...
                      {errors.gstCertificate && (
                        <p className="mt-1 text-sm text-red-600 flex items-center">
                          <AlertCircle className="w-4 h-4 mr-1" /> {errors.gstCertificate}
                        </p>
                      )}
                    </div>
                    ...
                    <div>
                      <label className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                        <Upload className="mr-2 text-green-600" /> Sales Invoices *
                      </label>
                      <input
                        type="file"
                        name="salesInvoices"
                        ...
                        className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-green-500"
                      />
                      {errors.salesInvoices && (
                        <p className="mt-1 text-sm text-red-600 flex items-center">
                          <AlertCircle className="w-4 h-4 mr-1" /> {errors.salesInvoices}
                        </p>
                      )}
                    </div>
                    ...
                    <div>
                      <label className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                        <Upload className="mr-2 text-green-600" /> Purchase Invoices *
                      </label>
                      <input
                        type="file"
                        ...
                        className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-green-500"
                      />
                      {errors.purchaseInvoices && (
                        <p className="mt-1 text-sm text-red-600 flex items-center">
                          <AlertCircle className="w-4 h-4 mr-1" /> {errors.purchaseInvoices}
                        </p>
                      )}
                    </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center">
              ...
              <p className="text-gray-600 mb-6">
                Your GST service application has been received. Our GST expert will contact you within 1-2 business days.
              </p>
              <button
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative pt-20 sm:pt-24 md:pt-28 lg:pt-32 pb-6 sm:pb-8 md:pb-10 lg:pb-12 min-h-[500px] sm:min-h-[550px] md:min-h-[580px] lg:h-[600px] bg-cover bg-center bg-no-repeat text-white flex items-center"
        style={{
          backgroundImage: "url('https://images.unsplash.com/photo-1554224154-26032ffc0d07?w=1600&q=80')",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8 lg:gap-10 items-center">
            <div className="space-y-3 sm:space-y-4 md:space-y-5 text-center md:text-left">
              <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold leading-tight">
                GST Services & <span className="text-yellow-300">Compliance</span>
              </h1>
              <p className="text-xs sm:text-sm md:text-base lg:text-lg text-white/95 leading-relaxed">
                End-to-end GST compliance services. From registration to annual returns, we handle all your GST requirements with expert guidance.
              </p>
              <div className="pt-2">
                <button 
                  onClick={() => {
      ...
      <div id="services" className="container mx-auto px-4 py-12 md:py-16">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-gradient-to-br from-green-600 to-teal-600 rounded-2xl md:rounded-3xl p-6 md:p-8 lg:p-12 text-white mb-12">
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-6 md:mb-8 text-center">GST Rate Structure</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              {gstRates.map((rate, idx) => (
                <div key={idx} className="bg-white/10 backdrop-blur-sm p-4 md:p-6 rounded-xl hover:bg-white/20 transition-all">
                  <div className="text-3xl md:text-4xl font-bold mb-2 text-yellow-300">{rate.rate}</div>
                  <p className="text-sm text-green-50">{rate.items}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Services Grid */}
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-8 text-center text-gray-800">
            Complete GST Solutions
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-12">
            {[
              { icon: FaBuilding, title: "Registration & Modifications", items: ["New GST registration", "Composition scheme", "Additional business places", "GST cancellation"] },
              { icon: FaFileInvoice, title: "Monthly Compliance", items: ["GSTR-1 filing", "GSTR-3B filing", "Invoice upload", "E-way bills"] },
              { icon: FaChartLine, title: "Annual Returns", items: ["GSTR-9 filing", "GSTR-9C reconciliation", "GST audit", "ITC reconciliation"] },
              { icon: FaMoneyBillWave, title: "Advisory & Refunds", items: ["Rate determination", "ITC optimization", "Export refunds", "Inverted duty refunds"] }
            ].map((service, idx) => (
              <div key={idx} className="bg-white p-6 md:p-8 rounded-xl shadow-lg hover:shadow-xl transition-all">
                <div className="flex items-start gap-4 mb-4">
                  <div className="bg-green-600 text-white p-4 rounded-lg">
                    <service.icon className="text-2xl" />
                  </div>
                  <h3 className="text-xl md:text-2xl font-bold text-gray-800 mt-2">{service.title}</h3>
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 lg:p-12 mb-12">
            ...
            <p className="text-base md:text-lg text-gray-600 leading-relaxed mb-4">
              Goods and Services Tax (GST) is a comprehensive indirect tax levied on the supply of goods and services in India. It replaced multiple indirect taxes like VAT, service tax, and excise duty, creating a unified tax system.
            </p>
            <p className="text-base md:text-lg text-gray-600 leading-relaxed">
              Our GST services ensure complete compliance with all GST regulations—from registration and monthly return filing to annual returns and GST audits. We help businesses navigate the complex GST framework efficiently.
            </p>
          </div>
            ...
            <div className="max-w-7xl mx-auto">
              <div className="text-center mb-12">
                ...
                <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                  Keep these documents ready for GST registration and compliance
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="bg-gradient-to-br from-green-50 to-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-green-100">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-gray-900 mb-2">Business Documents</h3>
                      <p className="text-gray-600 text-sm mb-3">
                        Company registration and business proof
                      </p>
                      <ul className="space-y-2">
                ...
                <div className="bg-gradient-to-br from-green-50 to-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-green-100">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-gray-900 mb-2">Identity Documents</h3>
                      <p className="text-gray-600 text-sm mb-3">
          ...
          <div className="text-center bg-white rounded-2xl shadow-xl p-8 md:p-12">
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-3 md:mb-4 text-gray-800">Need GST Assistance?</h2>
            <p className="text-gray-600 text-base md:text-lg mb-6 md:mb-8 max-w-2xl mx-auto">
              Stay compliant with expert GST services. From registration to annual returns, we've got you covered.
            </p>
            <button
